...
const REPO_NAME = 'nekooee/telegram-group-manager-bot';
...
<body>
  ...
  <script>
    ...
    form.addEventListener('submit', async (e) => {
      ...
      status.textContent = 'در حال دانلود و نصب ربات...'; status.style.color = '#f59e0b';
      try {
...
app.post('/install', async (req, res) => {
  ...
  try {
    ...
    if (!fs.existsSync(BOT_DIR)) {
      await execPromise(`npx degit ${REPO_NAME} ${BOT_DIR}`);
    }
...
app.listen(PORT, () => console.log('Installer Panel running on port ' + PORT));
